...
const topics: Topic[] = [
  ...
  },
  {
    ...
    summary: '在应用内下载账号文件，并把下载结果批量导入 Stone+。', page: 'browser',
...
const helpEnglish = new Map<string, string>([
  ...
  ['配置时从左到右准备；真正发请求时从客户端反向进入 Stone+。', 'Prepare the pieces from left to right. At request time, traffic enters Stone+ from the client in the opposite direction.'],
  ['本地网关', 'Local gateway'], ['路由与来源', 'Routes and sources'], ['自动检测', 'Automatic check'],

  // Quick start.
  ['第一次使用不必逐页研究。最省事的方法是打开新手向导，按页面提示准备一个可用来源，然后让 Stone+ 自动建立最小链路。', 'You do not need to study every page before using Stone+. The simplest path is to open the setup wizard, add one usable source, and let Stone+ build the minimum working request path.'],
  ['Stone+ 从向导到配置完成的操作演示', 'Stone+ setup wizard walkthrough'],
  ...
  ['把 Stone+ 想成一个本地总机：', 'Think of Stone+ as a local switchboard: '],
  ...
  ['点击本页顶部“打开新手向导”，先让系统扫描客户端目录和本机网络。', 'Use the button at the top of this page and let Stone+ scan client directories and the local network.'],
  ...
  ['选择客户端后点“一键修复连接”；Stone+ 会先备份，并只处理必要连接字段。', 'Choose the client and select Repair connection. Stone+ creates a backup first and only changes the fields required for the connection.'],
  ['现在开始配置', 'Start setup now'], ['检查当前环境', 'Check this environment'],
  ['Stone+ 新手向导界面', 'Stone+ setup wizard'],
  ...
  // Accounts and relays.
  ['“来源”是 Stone+ 能调用模型的凭据入口。一个来源可以是 ChatGPT 账号、官方 API Key，也可以是 OpenAI / Anthropic / Gemini 兼容中转。', 'A source is the credential Stone+ uses to call a model. It can be a ChatGPT account, an official API key, or an OpenAI-, Anthropic-, or Gemini-compatible relay.'],
  ...
  ['添加与验证', 'Add and verify'], ['OAuth 登录 ChatGPT', 'Sign in to ChatGPT with OAuth'], ['浏览器授权完成后自动回到 Stone+', 'Return to Stone+ automatically after browser authorization'],
  ...
  ['在浏览器完成授权', 'Complete authorization in the browser'], ['不要关闭 Stone+；授权页面完成后等待应用自动交换凭据。', 'Keep Stone+ open. After authorization, wait for the app to exchange the credentials.'],
  ...
  ['Stone+ 账号与中转管理页面', 'Stone+ accounts and relays page'], ['账号卡集中显示状态、配额、模型、并发、代理和所属标签；批量操作不会要求逐个打开。', 'Account cards show status, quota, models, concurrency, proxy, and tags together. Batch actions do not require opening every account.'],
  ...
  ['最大并发', 'Maximum concurrency'], ['限制同一账号同时处理的请求数，达到上限后调度其他账号或等待。', 'Limits simultaneous requests on one account. At the limit, Stone+ schedules another account or waits.'],
  ...
  ['Stone+ 号池与调度策略页面', 'Stone+ pools and scheduling page'], ['成员、模型范围、调度策略、粘性会话与重试集中配置；高级项按需展开。', 'Configure members, model scope, scheduling, sticky sessions, and retries together. Expand advanced controls only when needed.'],
  ...
  // Routes.
  ['路由回答三个问题：哪种客户端发来的请求、用哪种协议接收、交给哪个来源。路由保存但未启用时不会参与请求。', 'A route answers three questions: which client sent the request, which protocol Stone+ receives, and which source handles it. A saved but disabled route does not receive traffic.'],
  ...
  ['入站协议', 'Inbound protocol'], ['Stone+ 在本地接收的请求格式，通常按客户端默认值即可。', 'The format Stone+ accepts locally. The client default is usually correct.'],
  ...
  ['Stone+ 路由管理页面', 'Stone+ routes page'], ['路由卡会显示客户端、入站协议、目标来源、模型映射和启用状态。', 'Route cards show the client, inbound protocol, target source, model mappings, and enabled state.'],
  ...
  ['例如客户端请求一个固定别名，而中转只接受另一模型名，可以建立“请求模型 → 上游模型”的映射。没有映射时 Stone+ 会尽量保留原模型名。', 'If a client requests a fixed alias but the relay accepts a different model name, map requested model → upstream model. Without a mapping, Stone+ preserves the requested name when possible.'],
  ...
  ['客户端配置写入的是路由的本地令牌，请求到达 Stone+ 后才在本地安全存储中取上游凭据。不要把上游 OAuth Token 或 API Key 手工写进客户端配置。', 'Client files contain the route local token. Stone+ reads upstream credentials from secure local storage only after a request arrives. Never put an upstream OAuth token or API key directly in client configuration.'],
  ...
  ['“当前上游”显示这个客户端正在使用的号池、官方 API 或中转站。', 'Current upstream shows the pool, official API, or relay used by this client.'], ['“配置正常”表示必要连接字段已能把客户端指向当前 Stone+ 网关。', 'Configuration OK means the required fields point the client at the current Stone+ gateway.'], ['显示“配置损坏”或“尚未配置”时，直接使用“一键修复连接”。', 'If it reports a damaged or missing configuration, use Repair connection.'],
  ['切换上游不会再改坏客户端文件', 'Switching upstreams does not rewrite client files'], ['下拉选择新上游时，Stone+ 只原子更新内部路由；不会重写 ', 'Choosing a new upstream only updates the internal route atomically. It does not rewrite '], ['、', ', '], [' 或 ', ' or '], ['，客户端始终稳定连接本地 Stone+。', '; the client remains connected to local Stone+.'],
  ...
  ['选择出问题的客户端', 'Choose the affected client'], ['顶部 Tab 切换客户端，Stone+ 会自动检测默认配置目录和文件健康状态。', 'Use the tabs to switch clients. Stone+ detects the default directory and file health automatically.'],
  ['点击“一键修复连接”', 'Select Repair connection'], ['Stone+ 先备份现有文件；文件有效时只修复本地地址和本地令牌，模型、MCP、插件等用户设置保持不变。', 'Stone+ backs up existing files first. If they parse correctly, it changes only the local address and token; models, MCP, plugins, and other user settings remain intact.'],
  ['损坏文件自动重建', 'Rebuild damaged files safely'], ['若文件已经无法解析，会保留原始备份，再重建一份能连接 Stone+ 的最小配置。需要时可恢复修复前版本。', 'If a file cannot be parsed, Stone+ preserves the original and creates a minimal working configuration. You can restore the pre-repair version later.'],
  ...
  ['Stone+ 超级易用客户端配置', 'Stone+ client configuration page'], ['默认页只保留上游切换、连接状态和修复入口；完整编辑器收在高级设置中。', 'The default view shows upstream switching, connection status, and repair. Full editors are under Advanced settings.'],
  ['恢复与高级设置', 'Restore and advanced settings'], ['修复后想退回怎么办', 'How to undo a repair'], ['恢复最近备份', 'Restore the latest backup'],
  ['检测到需要写入时，Stone+ 会先把已有相关文件保存为同一备份组。“恢复最近备份”会先给当前状态再做一次安全快照，然后整组恢复，避免 config/auth 或 settings/env 混用不同版本。', 'Before writing, Stone+ saves related files as one backup group. Restore latest backup first snapshots the current state, then restores the group together so paired files never come from different versions.'],
  ...
  ['没有“恢复出厂配置”', 'There is no factory reset for client files'], ['Stone+ 不会用一套大而全的模板覆盖你的文件。“一键修复连接”只处理连接链路；无法解析时才重建最小文件，并且始终先留备份。', 'Stone+ never overwrites your files with a large generic template. Repair connection changes only the connection path; it creates a minimal file only when parsing fails, and always backs up first.'],
  ...
  ['什么时候使用', 'When to use it'], ['适合修复', 'Good repair candidate'], ['换到 Stone+ 后只有旧 Codex 会话报 provider、401 或关联错误；新会话正常。', 'After switching to Stone+, only old Codex conversations fail with provider, 401, or linkage errors while new conversations work.'],
  ...
  ['打开会话修复并扫描', 'Open Session Repair and scan'], ['系统会检查 Codex Home、rollout 文件、索引和数据库，先给出只读预览。', 'Stone+ checks Codex Home, rollout files, indexes, and databases, then presents a read-only preview.'],
  ['选择目标 provider', 'Choose the target provider'], ['确认目标是当前 Stone+ 配置，不要在不清楚来源时批量替换。', 'Confirm it matches the current Stone+ configuration. Do not run a bulk replacement if the source is unclear.'],
  ...
  ['修复后仍失败', 'If it still fails'], ['用新旧会话对比定位', 'Compare new and old conversations'], ['确认客户端已经完全重启。', 'Confirm the client was fully restarted.'], ['比较新会话与旧会话在请求记录中的模型、路由和错误。', 'Compare model, route, and error details for new and old conversations in Request History.'], ['若没有任何请求到达 Stone+，到客户端配置页一键修复连接。', 'If no request reaches Stone+, repair the client connection.'], ['若到达但上游 401 / 429，处理账号或配额，而不是重复修复会话。', 'If it arrives but returns upstream 401/429, fix the account or quota instead of repeating session repair.'],
  ['打开会话修复', 'Open Session Repair'],

  // Tunnel and built-in browser.
  ['内网穿透把本机 Stone+ 网关临时映射为其他设备能访问的地址。它适合短期远程使用或局域网外测试，不应替代长期、受控的服务部署。', 'A tunnel temporarily gives another device access to the local Stone+ gateway. It is suitable for short remote use or testing outside the LAN, not as a permanent managed deployment.'],
  ['适用场景', 'Use cases'], ['另一台设备临时调用', 'Temporary access from another device'], ['Stone+ 运行在主机，笔记本或测试机通过穿透地址访问。', 'Stone+ runs on the host while a laptop or test machine uses the tunnel URL.'],
  ...
  ['发送短请求', 'Send a short request'], ['在 Stone+ 请求记录中确认远程请求到达；测试完成立即停止穿透。', 'Confirm the remote request appears in Stone+ Request History, then stop the tunnel immediately after testing.'],
  ...
  ['监听所有网卡与穿透是两回事', 'Listening on all interfaces is not the same as tunneling'], ['Stone+ 为保护本地凭据只允许 127.0.0.1、localhost 或 ::1，不接受 0.0.0.0。需要其他设备访问时请使用受控的内网穿透，不要直接暴露网关端口。', 'Stone+ accepts only 127.0.0.1, localhost, or ::1 to protect local credentials; it does not accept 0.0.0.0. Use the controlled tunnel instead of exposing the gateway port.'],
  ...
  ['能做什么', 'What it can do'], ['网页访问', 'Browse pages'], ['在 Stone+ 内打开需要的账号管理 / 下载页面，保留当前操作上下文。', 'Open account management or download pages inside Stone+ and keep the current workflow together.'],
  ...
  ['Stone+ 内置浏览器与下载管理页面', 'Stone+ built-in browser and download manager'], ['浏览器主区与下载抽屉在同一页面；下载完成后可勾选文件批量送往账号导入。', 'The browser and download drawer share one page. Select completed files and send them to account import in a batch.'],
  ...
  ['Stone+ 一键诊断结果页面', 'Stone+ diagnostics results'], ['顶部给出整体结论，下方分别显示本地配置和网络目标；失败项带直接说明。', 'The summary appears at the top, followed by local and network checks. Failed items include a direct explanation.'],
  ...
  ['按结果修复', 'Fix by result'], ['DNS / 连接超时', 'DNS / connection timeout'], ['优先比较直连和代理', 'Compare direct and proxy access first'], ['检查本机网络与系统时间。', 'Check the local network and system clock.'], ['直连和目标代理各跑一次，比较是哪条出口失败。', 'Run both direct and target-proxy tests to identify the failing exit.'], ['确认代理协议 / 端口并检查防火墙。', 'Verify proxy protocol/port and firewall rules.'], ['网络恢复后重新诊断，Stone+ 会重建出站连接。', 'Run diagnostics again after recovery; Stone+ rebuilds outbound connections.'],
  ['TLS / 证书错误', 'TLS / certificate error'], ['检查代理拦截、系统时间与证书环境', 'Check proxy interception, system time, and trust store'], ['先关闭会做 HTTPS 解密的代理规则复测；公司网络需要自签根证书时，确认运行 Stone+ 的环境真正信任该证书。不要用关闭证书校验作为长期解决方案。', 'Retest without HTTPS-decryption proxy rules. If a corporate network uses a private root CA, ensure the environment running Stone+ trusts it. Do not disable certificate verification as a permanent fix.'],
  ...
  ['自动启动网关', 'Start gateway automatically'], ['常用时开启', 'Enable for regular use'], ['只偶尔使用或需要手工控制服务时关闭。', 'Disable if you use Stone+ rarely or want manual service control.'],
  ...
  ['打开更新入口', 'Open the updater'], ['从设置或更新提示打开对话框，查看当前版本、新版本和发布说明。', 'Open the dialog from Settings or an update prompt to see current/new versions and release notes.'], ['开始下载', 'Start download'], ['保持 Stone+ 运行；界面会显示进度、速度或阶段，不要重复点击。', 'Keep Stone+ running. Watch progress/speed/stage and do not click repeatedly.'], ['等待校验', 'Wait for verification'], ['下载完成后还需校验文件完整性，校验失败不会直接安装。', 'After download, Stone+ verifies integrity. A failed verification is not installed.'], ['保存工作并重启', 'Save work and restart'], ['确认没有进行中的请求或导入任务，再按提示重启完成更新。', 'Make sure no requests or imports are running, then restart when prompted.'], ['更新后验证', 'Verify after updating'], ['查看版本号，启动网关并发一个短请求，确认客户端配置仍匹配实际端口。', 'Check the version, start the gateway, and send a short request to confirm client configuration still matches the actual port.'],
  ['Stone+ 在线更新对话框', 'Stone+ update dialog'], ['更新对话框展示版本说明、下载 / 校验阶段和下一步按钮，不需要手工寻找安装包。', 'The dialog shows release notes, download/verification stages, and the next action without requiring you to find an installer manually.'],
  ...
  ['校验失败 / 安装后仍是旧版本', 'Verification failed / old version remains'], ['不要反复安装同一个损坏文件', 'Do not reinstall the same damaged file'], ['重新下载，确认安全软件没有截断或隔离安装文件。完全退出 Stone+ 后再运行安装包；便携版应替换正确目录而不是启动旧快捷方式。', 'Download again and ensure security software did not truncate or quarantine the installer. Fully exit Stone+ before installing. For portable builds, replace the correct directory rather than launching an old shortcut.'],
  ...
  ['提交问题时带什么信息', 'What to include in a bug report'], ['Stone+ 版本与系统、失败时间、客户端、诊断结果、请求状态码和已脱敏错误即可。不要附 OAuth Token、API Key、本地令牌、Cookie 或完整私人提示词。', 'Include the Stone+ version and OS, failure time, client, diagnostics result, request status code, and a redacted error. Never include OAuth tokens, API keys, local tokens, cookies, or full private prompts.'],
  ['网关启动失败，提示端口被占用怎么办？', 'The gateway says its port is already in use. What should I do?'], ['先在设置中换到相邻空闲端口并重启网关。随后到客户端配置页点击“一键修复连接”，让客户端改用实际端口。若必须使用原端口，先退出占用它的旧 Stone+ 或其他服务。', 'Choose a nearby free port in Settings and restart the gateway. Then use Repair connection so clients use the actual port. If the original port is required, stop the old Stone+ instance or other service using it.'], ['检查网关设置', 'Check gateway settings'],
  ...
  ['请求后一直显示 1 个活跃请求怎么办？', 'One active request remains after the response.'], ['先确认客户端是否仍在接收流式响应。若结果已经完整返回，刷新页面并更新到包含流结束修复的最新版本，然后重启网关。请求记录仍显示“流式”时展开详情；不要在正常长请求尚未结束时强制停止。', 'Confirm whether the client is still receiving a stream. If it already received the full result, refresh, update Stone+, and restart the gateway. Expand a record that remains Streaming; do not stop a legitimately active long request.'], ['查看最新请求', 'View latest request'],
  ['出现 401 / Unauthorized 怎么定位？', 'How do I diagnose 401 / Unauthorized?'], ['没有请求记录时多半是客户端使用了错误的本地令牌；有记录且错误来自上游时，刷新账号状态、重新 OAuth 登录或核对 API Key。不要把上游密钥直接写到客户端来绕过 Stone+。', 'No request record usually means the client has the wrong local token. If a record shows an upstream 401, refresh account status, sign in with OAuth again, or verify the API key. Never bypass Stone+ by putting upstream credentials in the client.'], ['区分本地与上游 401', 'Distinguish local vs. upstream 401'],
  ...
  ['一键修复连接会覆盖我原来的配置吗？', 'Will Repair connection overwrite my configuration?'], ['文件能正常解析时，只修改连接 Stone+ 所需的地址、provider 和本地令牌，模型、MCP、插件与未知字段会保留。文件已经损坏时会先完整备份原文，再重建最小可用文件；可随时恢复修复前版本。', 'When files parse correctly, only the address, provider, and local token needed for Stone+ are changed; models, MCP, plugins, and unknown fields remain. A damaged file is backed up in full before a minimal working file is created.'], ['检查并修复连接', 'Check and repair connection'],
  ...
  ['基础地址填服务商文档给出的 API 根地址，不要多拼一次', 'Use the API root from the provider documentation. Do not append'],
  ['下拉选择新上游时，Stone+ 只原子更新内部路由；不会重写', 'Choosing a new upstream only updates the internal route atomically. It does not rewrite'],
...
export function HelpView({ snapshot, api, navigate }: HelpViewProps) {
  ...
  return (
    <div className="page-stack help-page">
      ...
      <section className="help-library" aria-label={t('Stone+ 使用文档', 'Stone+ documentation')}>
...
function FlowDemo() {
  ...
  return <LocalizedDoc><div className="help-flow-demo" aria-label="请求从客户端经过网关、路由、号池到达来源的动画示意">
    ...
    <p>配置时从左到右准备；真正发请求时从客户端反向进入 Stone+。</p>
  </div></LocalizedDoc>
}
...
function QuickStartDoc({ navigate, onImage }: { navigate: (page: PageId) => void; onImage: (image: { src: string; alt: string }) => void }) {
  return <LocalizedDoc><>
    <div className="help-lead"><p>第一次使用不必逐页研究。最省事的方法是打开新手向导，按页面提示准备一个可用来源，然后让 Stone+ 自动建立最小链路。</p></div>
    <ScreenshotFigure src={demoGif} alt="Stone+ 从向导到配置完成的操作演示" caption="完整流程动图：环境扫描、选择来源、验证网络、建立路由、启动网关并写入客户端。" onImage={onImage} />

    <SectionHeading topic="quick-start" id="understand-flow">先理解一条请求</SectionHeading>
    <FlowDemo />
    <p className="help-copy">把 Stone+ 想成一个本地总机：<strong>来源</strong>提供账号或 API；<strong>号池</strong>决定多个账号怎么轮换；<strong>路由</strong>决定某个客户端该走哪个来源；<strong>网关</strong>在本机接收请求；最后由<strong>客户端配置</strong>把 Codex、Claude Code 或 Gemini CLI 指向网关。</p>
    ...
    <Steps items={[
      ...
      { title: '一键接好客户端', text: <>选择客户端后点“一键修复连接”；Stone+ 会先备份，并只处理必要连接字段。</> },
    ]} />
    ...
    <ScreenshotFigure src={setupScreenshot} alt="Stone+ 新手向导界面" caption="左侧显示完整步骤，主区域一次只要求完成当前任务；随时退出也会保留非敏感进度。" onImage={onImage} />
    ...
    <ScreenshotFigure src={overviewScreenshot} alt="Stone+ 总览页面" caption="日常首先看总览：网关状态、启用路由、账号健康和最近请求会集中显示。" onImage={onImage} />
  </></LocalizedDoc>
}

function ProvidersDoc({ navigate, onImage }: { navigate: (page: PageId) => void; onImage: (image: { src: string; alt: string }) => void }) {
  return <LocalizedDoc><>
    <div className="help-lead"><p>“来源”是 Stone+ 能调用模型的凭据入口。一个来源可以是 ChatGPT 账号、官方 API Key，也可以是 OpenAI / Anthropic / Gemini 兼容中转。</p></div>
    ...
    <Guide title="OAuth 登录 ChatGPT" summary="浏览器授权完成后自动回到 Stone+" open>
      <Steps items={[
        { title: '选择“登录 ChatGPT”', text: '可先选择账号标签、目标号池和网络出口。' },
        { title: '在浏览器完成授权', text: '不要关闭 Stone+；授权页面完成后等待应用自动交换凭据。' },
    ...
    <ScreenshotFigure src={accountsScreenshot} alt="Stone+ 账号与中转管理页面" caption="账号卡集中显示状态、配额、模型、并发、代理和所属标签；批量操作不会要求逐个打开。" onImage={onImage} />
...
function PoolsDoc({ navigate, onImage }: { navigate: (page: PageId) => void; onImage: (image: { src: string; alt: string }) => void }) {
  return <LocalizedDoc><>
    <div className="help-lead"><p>号池把多个账号包装成一个稳定来源。客户端不需要知道本次用了哪个账号；Stone+ 会按策略选择可用成员，并在允许时切换或重试。</p></div>
    ...
    <ScreenshotFigure src={poolsScreenshot} alt="Stone+ 号池与调度策略页面" caption="成员、模型范围、调度策略、粘性会话与重试集中配置；高级项按需展开。" onImage={onImage} />
...
function RoutesDoc({ navigate, onImage }: { navigate: (page: PageId) => void; onImage: (image: { src: string; alt: string }) => void }) {
  return <LocalizedDoc><>
    ...
    <div className="help-definition-list">
      ...
      <div><strong>入站协议</strong><span>Stone+ 在本地接收的请求格式，通常按客户端默认值即可。</span></div>
    ...
    <ScreenshotFigure src={routesScreenshot} alt="Stone+ 路由管理页面" caption="路由卡会显示客户端、入站协议、目标来源、模型映射和启用状态。" onImage={onImage} />

    <SectionHeading topic="routes" id="route-models">模型与协议</SectionHeading>
    <Guide title="模型映射什么时候用" summary="客户端模型名与上游模型名不同才需要">
      <p>例如客户端请求一个固定别名，而中转只接受另一模型名，可以建立“请求模型 → 上游模型”的映射。没有映射时 Stone+ 会尽量保留原模型名。</p>
    ...
    <Guide title="本地令牌与上游密钥的区别" summary="客户端只应拿到本地令牌" open>
      <p>客户端配置写入的是路由的本地令牌，请求到达 Stone+ 后才在本地安全存储中取上游凭据。不要把上游 OAuth Token 或 API Key 手工写进客户端配置。</p>
    </Guide>
...
function ClientsDoc({ navigate, onImage }: { navigate: (page: PageId) => void; onImage: (image: { src: string; alt: string }) => void }) {
  return <LocalizedDoc><>
    ...
    <ul className="help-check-list">
      ...
      <li><CheckCircle2 size={16} /><span>“配置正常”表示必要连接字段已能把客户端指向当前 Stone+ 网关。</span></li>
      <li><CheckCircle2 size={16} /><span>显示“配置损坏”或“尚未配置”时，直接使用“一键修复连接”。</span></li>
    </ul>
    <Callout title="切换上游不会再改坏客户端文件">下拉选择新上游时，Stone+ 只原子更新内部路由；不会重写 <code>config.toml</code>、<code>settings.json</code> 或 <code>.env</code>，客户端始终稳定连接本地 Stone+。</Callout>
    ...
    <Steps items={[
      { title: '选择出问题的客户端', text: '顶部 Tab 切换客户端，Stone+ 会自动检测默认配置目录和文件健康状态。' },
      { title: '点击“一键修复连接”', text: 'Stone+ 先备份现有文件；文件有效时只修复本地地址和本地令牌，模型、MCP、插件等用户设置保持不变。' },
      { title: '损坏文件自动重建', text: '若文件已经无法解析，会保留原始备份，再重建一份能连接 Stone+ 的最小配置。需要时可恢复修复前版本。' },
      { title: '重启客户端并验证', text: '大多数 CLI 只在启动时读取配置。完全退出后重新打开，再发送一条短请求。' },
    ]} />
    <ScreenshotFigure src={clientsScreenshot} alt="Stone+ 超级易用客户端配置" caption="默认页只保留上游切换、连接状态和修复入口；完整编辑器收在高级设置中。" onImage={onImage} />

    <SectionHeading topic="clients" id="client-backup">恢复与高级设置</SectionHeading>
    <Guide title="修复后想退回怎么办" summary="恢复最近备份" open>
      <p>检测到需要写入时，Stone+ 会先把已有相关文件保存为同一备份组。“恢复最近备份”会先给当前状态再做一次安全快照，然后整组恢复，避免 config/auth 或 settings/env 混用不同版本。</p>
    </Guide>
    ...
    <Callout tone="warning" title="没有“恢复出厂配置”">Stone+ 不会用一套大而全的模板覆盖你的文件。“一键修复连接”只处理连接链路；无法解析时才重建最小文件，并且始终先留备份。</Callout>
...
function SessionRepairDoc({ navigate }: { navigate: (page: PageId) => void }) {
  return <LocalizedDoc><>
    ...
    <div className="help-scenario-grid">
      <div><Wrench size={18} /><strong>适合修复</strong><p>换到 Stone+ 后只有旧 Codex 会话报 provider、401 或关联错误；新会话正常。</p></div>
    ...
    <Steps items={[
      ...
      { title: '选择目标 provider', text: '确认目标是当前 Stone+ 配置，不要在不清楚来源时批量替换。' },
    ...
    <Guide title="修复后仍失败" summary="用新旧会话对比定位">
      <ol><li>确认客户端已经完全重启。</li><li>比较新会话与旧会话在请求记录中的模型、路由和错误。</li><li>若没有任何请求到达 Stone+，到客户端配置页一键修复连接。</li><li>若到达但上游 401 / 429，处理账号或配额，而不是重复修复会话。</li></ol>
    </Guide>
...
function TunnelDoc({ navigate }: { navigate: (page: PageId) => void }) {
  return <LocalizedDoc><>
    <div className="help-lead"><p>内网穿透把本机 Stone+ 网关临时映射为其他设备能访问的地址。它适合短期远程使用或局域网外测试，不应替代长期、受控的服务部署。</p></div>
    <SectionHeading topic="tunnel" id="tunnel-use">适用场景</SectionHeading>
    <div className="help-choice-grid">
      <div><MonitorCog size={18} /><strong>另一台设备临时调用</strong><p>Stone+ 运行在主机，笔记本或测试机通过穿透地址访问。</p></div>
    ...
    <Steps items={[
      ...
      { title: '发送短请求', text: '在 Stone+ 请求记录中确认远程请求到达；测试完成立即停止穿透。' },
    ]} />
    ...
    <Callout tone="warning" title="监听所有网卡与穿透是两回事">Stone+ 为保护本地凭据只允许 127.0.0.1、localhost 或 ::1，不接受 0.0.0.0。需要其他设备访问时请使用受控的内网穿透，不要直接暴露网关端口。</Callout>
    {tunnelSupported
...
function BrowserDoc({ navigate, onImage }: { navigate: (page: PageId) => void; onImage: (image: { src: string; alt: string }) => void }) {
  return <LocalizedDoc><>
    ...
    <div className="help-definition-list">
      <div><strong>网页访问</strong><span>在 Stone+ 内打开需要的账号管理 / 下载页面，保留当前操作上下文。</span></div>
    ...
    <ScreenshotFigure src={browserScreenshot} alt="Stone+ 内置浏览器与下载管理页面" caption="浏览器主区与下载抽屉在同一页面；下载完成后可勾选文件批量送往账号导入。" onImage={onImage} />
...
function DiagnosticsDoc({ navigate, onImage }: { navigate: (page: PageId) => void; onImage: (image: { src: string; alt: string }) => void }) {
  return <LocalizedDoc><>
    ...
    <ScreenshotFigure src={diagnosticsScreenshot} alt="Stone+ 一键诊断结果页面" caption="顶部给出整体结论，下方分别显示本地配置和网络目标；失败项带直接说明。" onImage={onImage} />
    ...
    <Guide title="DNS / 连接超时" summary="优先比较直连和代理">
      <ol><li>检查本机网络与系统时间。</li><li>直连和目标代理各跑一次，比较是哪条出口失败。</li><li>确认代理协议 / 端口并检查防火墙。</li><li>网络恢复后重新诊断，Stone+ 会重建出站连接。</li></ol>
    </Guide>
    <Guide title="TLS / 证书错误" summary="检查代理拦截、系统时间与证书环境">
      <p>先关闭会做 HTTPS 解密的代理规则复测；公司网络需要自签根证书时，确认运行 Stone+ 的环境真正信任该证书。不要用关闭证书校验作为长期解决方案。</p>
    </Guide>
...
function UpdatesDoc({ navigate, onImage }: { navigate: (page: PageId) => void; onImage: (image: { src: string; alt: string }) => void }) {
  return <LocalizedDoc><>
    ...
    <Steps items={[
      ...
      { title: '开始下载', text: '保持 Stone+ 运行；界面会显示进度、速度或阶段，不要重复点击。' },
    ...
    <ScreenshotFigure src={onlineUpdateScreenshot} alt="Stone+ 在线更新对话框" caption="更新对话框展示版本说明、下载 / 校验阶段和下一步按钮，不需要手工寻找安装包。" onImage={onImage} />
    ...
    <Guide title="校验失败 / 安装后仍是旧版本" summary="不要反复安装同一个损坏文件">
      <p>重新下载，确认安全软件没有截断或隔离安装文件。完全退出 Stone+ 后再运行安装包；便携版应替换正确目录而不是启动旧快捷方式。</p>
    </Guide>
...
const faqItems: FaqItem[] = [
  {
    ...
    answer: <>先在设置中换到相邻空闲端口并重启网关。随后到客户端配置页点击“一键修复连接”，让客户端改用实际端口。若必须使用原端口，先退出占用它的旧 Stone+ 或其他服务。</>,
  ...
  },
  {
    ...
    answer: <>没有请求记录时多半是客户端使用了错误的本地令牌；有记录且错误来自上游时，刷新账号状态、重新 OAuth 登录或核对 API Key。不要把上游密钥直接写到客户端来绕过 Stone+。</>,
  ...
  },
  {
    ...
    answer: <>文件能正常解析时，只修改连接 Stone+ 所需的地址、provider 和本地令牌，模型、MCP、插件与未知字段会保留。文件已经损坏时会先完整备份原文，再重建最小可用文件；可随时恢复修复前版本。</>,
...
function FaqDoc({ navigate }: { navigate: (page: PageId) => void }) {
  return <LocalizedDoc><>
    ...
    <Callout title="提交问题时带什么信息">Stone+ 版本与系统、失败时间、客户端、诊断结果、请求状态码和已脱敏错误即可。不要附 OAuth Token、API Key、本地令牌、Cookie 或完整私人提示词。</Callout>
  </></LocalizedDoc>
}
