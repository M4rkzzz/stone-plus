...
- 合并 thok404 的 #15：启用“禁用冷却”后，中转站的 429、额度耗尽信号和 Retry-After 不再使其账号进入全局冷却；上游错误状态仍如实返回。
- 开启该设置时，恢复已因失败或中转站额度信号而冷却的账号，并清理中转站过期的通用额度状态。
- 官方 OAuth/API 来源继续保留额度耗尽保护；凭证失效导致的停用仍不受“禁用冷却”设置覆盖。
