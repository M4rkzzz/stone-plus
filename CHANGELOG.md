...
- 完善 DSH 网关与长上下文：提供隔离的 Chat Completions、Responses、模型目录、会话模型过滤及搜索桥，
  真实传递会话 ID、工具定义、并行调用、结构化结果和终止语义；Chat 与 Responses 历史均使用受控
  10–64 MiB 大请求通道，保持共享内存门控和明确 413 边界。
