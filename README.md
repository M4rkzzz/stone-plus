...
<p align="center">
  <strong>更快的流式网关、更聪明的账号调度，以及批量导入、网络诊断与 Codex 会话修复。</strong>
</p>
...
Stone+ 基于[上游 Stone](https://github.com/EasyCode-Obsidian/Stone)，保留其多厂商接入、协议转换、模型路由和客户端配置能力，重点增强网关性能、账号调度、批量管理、网络适配与运行诊断。项目由社区独立维护，与上游官方无隶属或背书关系。
...
- **低延迟流式网关。** HTTP/2、连接复用与预热、SSE 提前转发、首正文超时切换和可选竞速请求，让首字更快、长连接更稳。
- **智能号池调度。** `autobalanced` 根据历史成功率、EWMA 首字、输出速度、额度和并发选账号，并用 **SP 体质分**展示账号长期表现；同时支持 Fast On 与额度解冻探测。
- **完整的账号批量工作流。** 支持 CPA/Sub2API/Codex JSON 批量导入导出、JWT 账号 ID 修复、批量出口代理，以及“导入 → 状态/额度刷新 → 可用模型查询”总体进度。
- **内置浏览器与 JSON 队列。** 支持多标签页、缩放和快捷入口；下载的账号 JSON 自动挂起，选完后统一导入。
- **系统代理与一键诊断。** 可直接跟随 Windows/PAC/Clash 系统代理，并检查 ChatGPT、Codex、OAuth、DNS、TLS、路由、账号与号池，给出可能原因。
- **更强的观测能力。** 提供详细阶段耗时、首字/首 Token、输出速度趋势，以及按模型和缓存规则计算的今日/累计 Token 成本。
- **Codex 会话修复与 FRP。** 可预览、备份并事务修复历史会话索引；也可直接运行内置 `frpc` 暴露本地网关。
