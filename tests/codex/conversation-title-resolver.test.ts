...
describe('CodexConversationTitleResolver', () => {
  it('reads the current title from the local Codex thread database', async () => {
    ...
    const resolver = new CodexConversationTitleResolver(codexDirectory)
  ...
  it('does not fail when Codex has no local database', async () => {
    ...
    temporaryDirectories.push(home)
    const resolver = new CodexConversationTitleResolver(join(home, '.codex'))
    expect(resolver.resolve('thread-one')).toBeUndefined()
    resolver.close()
  })

  it('reads titles from a config-selected sqlite_home', async () => {
    const home = await mkdtemp(join(tmpdir(), 'stone-codex-title-relocated-'))
    temporaryDirectories.push(home)
    const codexDirectory = join(home, '.codex')
    const sqliteHome = join(home, 'codex-state')
    await Promise.all([mkdir(codexDirectory), mkdir(sqliteHome)])
    await writeFile(
    ...
    database.exec('CREATE TABLE threads (id TEXT PRIMARY KEY, title TEXT NOT NULL)')
    database.prepare('INSERT INTO threads (id, title) VALUES (?, ?)').run('external-thread', 'External title')
    database.close()

    const resolver = new CodexConversationTitleResolver(codexDirectory)
    expect(resolver.resolve('external-thread')).toBe('External title')
    resolver.close()
  })
})
