...
describe('dark theme CSS contract', () => {
  ...
  it('defines theme-aware focus and control highlight tokens for both palettes', () => {
    ...
    expect(lightPalette).toContain('--scroll-thumb-active:')
    ...
    expect(darkPalette).toContain('--scroll-thumb: rgb(235 237 236 / 16%)')
    expect(darkPalette).toContain('--scroll-thumb-hover: rgb(235 237 236 / 30%)')
    expect(darkPalette).toContain('--scroll-thumb-active: rgb(255 255 255 / 42%)')
    expect(lightPalette).toContain('--on-accent: #ffffff')
    expect(lightPalette).toContain('--accent-text: var(--accent)')
    expect(lightPalette).toContain('--accent-rgb: 23 107 82')
    expect(lightPalette).toContain('--shadow-rgb: 20 33 28')
    expect(lightPalette).toContain('--overlay-rgb: 15 24 21')
    expect(lightPalette).toContain('--account-available-text: var(--ok-text)')
    expect(darkPalette).toContain('--on-accent: #151617')
    expect(darkPalette).toContain('--accent-text: #ff9000')
    expect(darkPalette).toContain('--accent-rgb: 255 144 0')
    expect(darkPalette).toContain('--accent-glow-rgb: 255 163 26')
    expect(darkPalette).toContain('--shadow-rgb: 0 0 0')
    expect(darkPalette).toContain('--overlay-rgb: 0 0 0')
    expect(darkPalette).toContain('--accent-text-strong: #ffa31a')
    expect(darkPalette).toContain('--account-available-text: #6cc9a5')
    expect(darkPalette).toContain('--bg: #111214')
    expect(darkPalette).toContain('--surface: #18191b')
    expect(darkPalette).toContain('--text: #fafafa')
    expect(darkPalette).toContain('--text-soft: #e4e5e6')
    expect(darkPalette).toContain('--muted: #c2c4c6')
    expect(darkPalette).toContain('--nav-text: #e1e3e2')
    expect(darkPalette).not.toContain('#5fbf9c')
    expect(focusRule).toContain('box-shadow: 0 0 0 3px var(--focus-ring)')
  })

  it('uses slim trackless scrollbars for the page and nested workspaces', () => {
    expect(ruleBody(globalCss, '*::-webkit-scrollbar')).toContain('width: 10px')
    expect(ruleBody(globalCss, '*::-webkit-scrollbar')).toContain('height: 10px')
    expect(ruleBody(globalCss, '*::-webkit-scrollbar-thumb')).toContain('border: 3px solid transparent')
    expect(ruleBody(globalCss, '*::-webkit-scrollbar-thumb')).toContain('background-clip: padding-box')
    expect(ruleBody(globalCss, '*::-webkit-scrollbar-thumb:hover')).toContain('background-color: var(--scroll-thumb-hover)')
    expect(ruleBody(globalCss, '.page-content::-webkit-scrollbar-thumb')).toContain('background: transparent')
    expect(ruleBodies(globalCss, '.page-content:hover::-webkit-scrollbar-thumb')).toContain('background-color: var(--scroll-thumb)')
  })
  ...
  it('does not reintroduce translucent white cards in help, telemetry, or shared UI', () => {
    expect(ruleBody(helpCss, '.help-page')).toContain('--help-accent-text: var(--accent-text, #176b52)')
    expect(helpCss).not.toContain('color: var(--help-green)')
  ...
  it('uses contrast foregrounds and semantic state fills', () => {
    ...
    expect(ruleBody(globalCss, "html[data-theme='dark'] .account-status-badge.badge--success")).toContain('color: var(--account-available-text)')
  })
})
