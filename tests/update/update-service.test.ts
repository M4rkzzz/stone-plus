...
describe('UpdateService', () => {
  ...
  it('opens only fixed project source and legal pages', async () => {
    const { service, openExternal } = createHarness()

    await service.openProjectPage('source')
    await service.openProjectPage('license')
    await service.openProjectPage('notices')
    await service.openProjectPage('trademarks')

    expect(openExternal.mock.calls.map(([url]) => url)).toEqual([
      'https://github.com/M4rkzzz/stone-plus',
      'https://github.com/M4rkzzz/stone-plus/blob/main/LICENSE',
