...
本文档用于 Stone+ `v0.8.0` 的中文首发、社区介绍和视频制作。文案中的链接均为项目正式地址，可直接使用。发布前只需确认 GitHub Release 已公开、下载文件与版本号一致。
...
- 源码可见许可证：https://github.com/M4rkzzz/stone-plus/blob/main/LICENSE
- QQ 交流群：`1061282900`
...
Stone+ 是一个面向个人开发者的、本地优先的多厂商 AI 账号与编程客户端控制中心，同时提供仅监听本机的 API 网关。
...
Stone+ 是一个运行在 Windows、macOS 和 Linux 上、源码公开可审计的本地控制中心。你可以添加自己有权使用的不同厂商账号，按账号拉取和测试模型，将账号组成号池，再让 Claude Code、Codex 或 Gemini CLI 通过同一个本地网关使用它们。Stone+ 会结合模型支持情况、额度、健康状态和号池策略选择可用账号，也支持常用协议转换、账号或号池级出口代理、客户端配置备份与恢复。
...
1. `[项目自荐] Stone+：把多厂商 AI 账号、号池和编程客户端放进一个本地桌面应用`
2. `做了一个本地 AI 账号与 Claude Code / Codex / Gemini CLI 控制中心`
3. `Stone+ v0.8.0：本地管理账号、模型、号池、代理与协议转换`

V2EX 可发布在“分享创造”节点。Linux.do 按站内当时的分类规则选择开发工具或软件分享相关分类。不要在标题里堆叠“神器”“全能”“颠覆”等词。

### 正文

大家好，我最近做了一个源码公开可审计的桌面项目：**Stone+**。
...
需要提前说明，当前 Windows 包使用项目持续自签名证书进行 Authenticode 签名，但该证书不在 Microsoft 公共信任链中；macOS 包尚未经过 Apple 公证，系统仍可能显示 SmartScreen、未知发布者或首次启动警告。Release 同时提供公钥证书、`SHA256SUMS` 与 GitHub 构建来源证明，建议下载后先核对。
...
StonePlus 自有源代码采用 StonePlus Source Available License 1.0，允许查看、审计、学习、有限本地测试和安全研究，但禁止修改、衍生产品、再分发、商业发行或对外托管、AI 训练或自动代码生成；上游及第三方代码继续适用各自原许可证。欢迎通过 Issues 或 QQ 群反馈具体环境、客户端、上游类型和复现步骤。
...
- `Stone+ 的实现：本地 AI 网关、模型策略与跨协议流式转换`
...
8. **跨平台发布与在线更新**
   ...
   - 坦诚说明 Windows 使用非公共信任的项目自签名证书，且 macOS 尚未公证。
...
>
> 这是 Stone+，一个运行在 Windows、macOS 和 Linux 上、源码公开可审计的本地控制中心。
>
...
>
> Stone+ 是个人本地工具，不是团队共享或计费平台。StonePlus 自有源码公开供审计，但不授权修改、衍生产品、再分发或 AI 自动生成；下载和当前限制都在 GitHub Release。
...
| 6–13 秒 | Stone+ 总览页，停留在网关状态和统计区域 | Stone+ · 本地源码公开可审计 |
...
| 74–82 秒 | 在线更新弹窗，再切 GitHub 项目页与 Release 页 | Source Available 1.0 · GitHub / Issues / QQ群 1061282900 |

### 视频简介

Stone+ 是一个面向个人开发者的本地 AI 账号、号池、代理与编程客户端控制中心，支持 Claude Code、Codex、Gemini CLI，以及 OpenAI、Anthropic、Gemini 常用协议的本地转发与转换。
...
当前 Windows 使用非公共信任的项目自签名证书，macOS 尚未公证，请从 GitHub Release 下载并核对证书指纹、`SHA256SUMS` 与构建来源证明。用户应只接入自己有权使用的账号，并遵守对应厂商条款。

## 短帖版本

### 通用短帖

Stone+ `v0.8.0`：一个运行在 Windows、macOS 和 Linux 上的本地 AI 账号与编程客户端控制中心。
...
Stone+ `v0.8.0` 发布：在本机统一管理多厂商 AI 账号、账号级模型、号池、HTTP/SOCKS 出口代理，以及 Claude Code、Codex、Gemini CLI 配置与路由。支持常用协议转换，数据留在本机。GitHub：https://github.com/M4rkzzz/stone-plus

### Release 转发版本

Stone+ `v0.8.0` 已发布：Windows、macOS、Linux 多平台构建，新增 GitHub Releases 更新检查、版本忽略、下载进度和更新说明。`v0.7.1` 用户需手工安装 `v0.8.0` 一次。下载：https://github.com/M4rkzzz/stone-plus/releases/latest

### QQ 群公告版本

Stone+ `v0.8.0` 已发布。请只从 GitHub Release 下载，并核对 `SHA256SUMS`：https://github.com/M4rkzzz/stone-plus/releases/latest
...
当前 Windows 包带项目持续自签名 Authenticode 签名与时间戳，但证书不在 Microsoft 公共信任链中；macOS 包尚未经过 Apple 公证，因此系统仍可能显示 SmartScreen、未知发布者或首次启动警告。这是当前发布限制，不应被隐藏。请只从项目 GitHub Release 下载，并核对公钥证书指纹、`SHA256SUMS` 与 GitHub 构建来源证明。公共信任签名和 Apple 公证仍在后续计划中。
...
不同安装形式的安全替换机制不同。当前只对 Windows 安装版和 Linux AppImage 开放应用内下载与重启安装。Portable、deb 和仅作 ad-hoc 签名且尚未公证的 macOS 构建会打开可信的 GitHub Release 页面，让用户手工完成替换，避免假装支持一个没有充分验证的安装流程。
...
StonePlus 自有源代码根据 StonePlus Source Available License 1.0 公开供查看、审计、学习、有限本地测试和安全研究；禁止修改、衍生产品、Fork 重包装、再分发、商业发行或对外托管、AI 训练和自动代码生成。上游及第三方组件继续适用各自许可证；Stone+、StonePlus、Logo、官方更新和签名身份另受品牌政策保护。上游模型或账号产生的费用与使用规则不属于 Stone+ 许可证范围。
...
- 在干净环境验证 Windows 安装版/Portable、Linux AppImage/deb；记录 macOS ad-hoc 签名但未公证时的实际提示。
...
- [ ] Release notes 明确 Windows 自签名证书不受公共信任、macOS 未公证和各安装形式的更新方式。
